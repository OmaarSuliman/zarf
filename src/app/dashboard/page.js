...
const fontOptions = [
    ...
    // Arabic Fonts
    { value: 'var(--font-lemonada)', label: 'Arabic - Lemonada' },
    { value: 'var(--font-amiri)', label: 'Arabic - Amiri' },
    { value: 'var(--font-el-messiri)', label: 'Arabic - El Messiri' },
    { value: 'var(--font-tajawal)', label: 'Arabic - Tajawal' },
];
...
export default function CreateNewZarfPage() {
  ...
  const handleFontChange = (selectedValue) => {
    // Set the same font for both messages
    handleStateChange('fontOne', selectedValue);
    handleStateChange('fontTwo', selectedValue);
  };
      ...
      <div className="mt-8 flex flex-col lg:flex-row gap-8 items-start">
        <div className="w-full lg:w-1/3 lg:sticky top-28 order-1 lg:order-2">
          ...
          <div className="relative w-full aspect-[16/10] bg-gradient-to-br from-zinc-100 to-zinc-200 rounded-2xl flex items-center justify-center p-4 border border-zinc-200/80 overflow-hidden">
            {/* --- SIMPLIFIED FONT STYLE --- */}
            <div className="relative w-[90%] h-[90%] bg-white rounded-lg shadow-xl flex flex-col items-center justify-center p-6 text-center" style={{ fontFamily: formState.fontOne }}>
            ...
            <motion.div key={currentStep} variants={stepVariants} initial="hidden" animate="visible" exit="exit" transition={{ duration: 0.3, ease: 'easeInOut' }}>
              {currentStep === 0 && (
                <div className="space-y-8">
                  ...
                  {/* --- UNIFIED FONT DROPDOWN --- */}
                  <AnimatedDropdown label="Font Style (for both sides)" options={fontOptions} value={formState.fontOne} onChange={handleFontChange} />
                </div>
              )}
              {currentStep === 1 && (
                 <div className="space-y-8">
                  ...
                  {/* The font dropdown is removed from this step for simplicity */}
                </div>
              )}
              {currentStep === 2 && (
