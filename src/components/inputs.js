...
export const AnimatedInput = ({
  ...
  const ringClass = "border-gray-300 focus:border-blue-500";
  const labelClass = "text-gray-500 peer-focus:text-blue-600";

  return (
    <div className="relative w-full">
        <label
            htmlFor={label}
            className={`absolute left-2 -top-2.5 text-sm transition-all
            bg-white px-1
            ${labelClass}
            `}
        >
        ...
        <input
            ...
            placeholder={placeholder}
            disabled={disabled}
            className={`peer w-full px-4 py-3 text-base bg-white border rounded-xl appearance-none
